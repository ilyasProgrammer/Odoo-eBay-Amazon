...
class DbBackup(models.Model):
        ...
        for rec in self.filtered("days_to_keep"):
            with rec.cleanup_log():
                ...
                if rec.method == "local":
                    ...
                    remote = rec.sftp_connection()
                    for name in remote.nlst(rec.folder):
                        if (name.endswith(".dump.zip") and
                                        os.path.basename(name) < oldest):
                            res = remote.delete('%s/%s' % (rec.folder, name))
                            _logger.info("FTP delete reply: %s", res)
                    remote.close()
    ...
    def sftp_connection(self):
        ...
        return ftplib.FTP(params["host"], params["username"], params["password"])
